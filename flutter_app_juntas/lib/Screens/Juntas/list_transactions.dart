...
class _ListTransactionsState extends State<ListTransactions> {
  ...
  getCurrentUser() async {
    ...
    _JuntaTransactionQuery = await _database
        .child("Juntas_Transactions/"+widget.junta_code);
    _onJuntaTransactionAddedSubscription = _JuntaTransactionQuery.onChildAdded.listen(onEntryAdded);
  ...
  deleteJuntaTransaction(String JuntaTransactionId, int index) async {
    await _database.reference().child("Juntas_Users/"+widget.user_.Id).child(JuntaTransactionId).remove().then((_) {
        ...
        child: Scrollbar(
          ...
          child: ListView.builder(
              ...
              itemBuilder: (BuildContext context, int index) {
                ...
                String code = _JuntaTransactionList[_JuntaTransactionList.length-index-1].key;
                String aporte = _JuntaTransactionList[_JuntaTransactionList.length-index-1].aporte;
                String aporte_day =_JuntaTransactionList[_JuntaTransactionList.length-index-1].aporte_day;
                String last_name =_JuntaTransactionList[_JuntaTransactionList.length-index-1].last_name;
                String name_integrant= _JuntaTransactionList[_JuntaTransactionList.length-index-1].name_integrant;
                String state_pay= _JuntaTransactionList[_JuntaTransactionList.length-index-1].state_pay;
                Color color;
                String color_state="";
                      ...
                      child: ListTile(
                        ...
                        leading: Container(
                          ...
                          decoration: new BoxDecoration(
                              border: new Border(
                                  right: new BorderSide(width: 2.0, color: Colors.lightBlue)
                              )
                          ),
                          child: Icon(Icons.person, color: color,size: 25,),
                        ),
                        ...
                        trailing:
                        Text(color_state+" "+aporte+ " " + widget.junta_info.Coin_type.split(" ")[widget.junta_info.Coin_type.split(" ").length - 1],
                          style: TextStyle(fontSize: 15.0, color: color),
                        ),
                        onTap: () async {
                          final ConfirmAction action = await _asyncConfirmDialog(context, aporte_day);
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
        ...
        appBar: AppBar(
          backgroundColor: kPrimaryLightColor,
          title: new Text("Historial de Junta", style: TextStyle(color: kPrimaryColor, fontSize: 22,)),
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                        ...
                        RichText(
                          text: TextSpan(
                            children: [
                              if(widget.junta_info.Coin_type=="Tipo de Moneda: Dólares")TextSpan(text:  "\$. "),
                              if(widget.junta_info.Coin_type=="Tipo de Moneda: Soles")TextSpan(text:  "S/. "),

                              TextSpan(
                                text: widget.junta_info.Total_amount.toStringAsFixed(0),
                        ...
                        Row(
                          ...
                          children: <Widget>[
                            ...
                            SizedBox(width: 5.0),
                            if(widget.junta_info.Coin_type=="Tipo de Moneda: Dólares")Text("Aporte " + widget.junta_info.tipoJunta +  ": \$. "+ widget.junta_info.Aporte.toStringAsFixed(0), style: TextStyle(color: Colors.grey[300]),),
                            if(widget.junta_info.Coin_type=="Tipo de Moneda: Soles")Text("Aporte " + widget.junta_info.tipoJunta +  ": S/. "+ widget.junta_info.Aporte.toStringAsFixed(0), style: TextStyle(color: Colors.grey[300]),)
